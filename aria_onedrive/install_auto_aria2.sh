...
# 脚本源码路径
onedrive_script_path="https://raw.githubusercontent.com/0oVicero0/OneDrive/master/Business"
custom_onedrive_script_path="https://67zz.cn/shell"
...
download_aria2_conf(){
     mkdir "${file}" && cd "${file}"
	 wget --no-check-certificate -N "${custom_onedrive_script_path}/aria2.conf"
...
service_aria2(){
     if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate ${custom_onedrive_script_path}/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate ${custom_onedrive_script_path}/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
install_autoUpload(){
	...
	mkdir -p ${auto_upload}
    cd ${auto_upload}
	wget --no-check-certificate -q -O json-parser "${onedrive_script_path}/json-parser"
	wget --no-check-certificate -q -O onedrive "${onedrive_script_path}/onedrive"
	wget --no-check-certificate -q -O onedrive-d "${onedrive_script_path}/onedrive-d"
	wget --no-check-certificate -q -O onedrive-authorize "${custom_onedrive_script_path}/onedrive-authorize"
	wget --no-check-certificate -q -O onedrive-base "${onedrive_script_path}/onedrive-base"
	wget --no-check-certificate -q -O onedrive.cfg "${custom_onedrive_script_path}/onedrive.cfg"
	chmod -R a+x ${auto_upload}
	ln -sf ${auto_upload}/onedrive /usr/local/bin/
	ln -sf ${auto_upload}/onedrive-d /usr/local/bin/
    rm -rf $(basename "$0")
    if [ -d "${file}" ]; then
		wget -P ${file}  --no-check-certificate -N "${custom_onedrive_script_path}/autoUpload.sh"
		sed -i '$a on-download-complete=${file}/autoUpload.sh' "${aria2_conf}"
		chmod 777 ${file}/*
		restart_aria2
	fi
